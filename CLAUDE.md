...
#### 阶段 6: Creem 支付集成 - 调试中
- [x] 增强调试日志
- [ ] 解决 Creem API 500 错误 ⚠️（见下方"已知问题"）
- [ ] 完整支付流程测试
...
### 🐛 Creem API 500 错误（进行中）

**问题描述**:
- 在 Pricing 页面点击购买按钮后，后端调用 Creem API 创建 checkout session 失败
- Creem API 返回 `500 Internal Server Error`
- 错误响应只包含状态码和时间戳，没有具体错误信息
...
**已尝试的修复**:
1. ✅ 使用真实的 Creem Product IDs（从 Dashboard 获取）
2. ✅ 验证 API Key 格式和权限
3. ✅ 添加详细的请求日志
4. ✅ 移除可能不支持的 `mode: 'payment'` 字段
5. ✅ 检查请求 payload 格式

**可能的原因**:
1. **Product 配置错误**: Creem Dashboard 中的产品可能配置为 "Subscription" 模式，但请求按一次性付款处理
2. **缺少必需字段**: Creem 标准接口要求 `mode` + `line_items`/`items` + `quantity`，还可能需要 `customer_email` 等字段，当前 payload 未包含这些数据
3. **URL 格式问题**: `success_url` 和 `cancel_url` 使用 localhost 可能不被接受
4. **API Key 权限**: Test mode API key 可能没有创建 checkout session 的权限
5. **Creem 服务端问题**: 可能是 Creem 的服务器暂时性问题

**最新排查记录 (2025-10-03)**:
- 根据 Creem Checkout 文档，`/v1/checkout/sessions` 在 `mode` 为订阅时至少需要 `line_items`/`items` 数组和 `quantity`。
- 当前 `server/index.js:176` 仅发送 `product_id`、`success_url`、`cancel_url` 和 `metadata`，缺少上述必填字段，极可能导致 500。
- 建议扩展 `server/creem-products.js` 暴露 `price_id` 映射，或改用 `items: [{ product_id, quantity: 1 }]`，并重新写入 `mode: 'subscription'`、`customer_email` 等信息。
...
这样可以先测试完整的积分系统，跳过真实支付流程。

**下一步调试**:
1. 更新 `server/index.js` 构造的 checkout payload，补充 `mode`、`line_items`/`items`、`quantity` 以及必要的 `customer_email`。
2. 若需要 price 维度，扩展 `server/creem-products.js` 提供 `price_id` 映射或在 metadata 中标记 plan。
3. 使用 Postman/Insomnia 手动调用 Creem API，验证新 payload 返回 200 并记录响应。
4. 本地通过 Pricing 页面复测流程，并确认 `checkout.session.completed` webhook 能写入积分/订阅。
5. 若仍报错，将最新请求日志提交给 Creem 支持以定位服务端问题。

**相关文件**:
- `server/index.js:164-228` - Checkout API 实现
- `server/creem-products.js` - Product ID 映射
- `src/pages/Pricing.tsx:23-45` - 前端购买逻辑
